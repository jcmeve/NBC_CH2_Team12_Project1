...
void DisplayManager::Render(float deltaTime) {
	...
	for (int i = 0; i < stringDeque.size(); ++i) {
		short offset = 0;
		for (int j = 0; j < stringDeque[i].size(); ++j) {
			DrawWcharAtPosition(j + offset, borderline + 1 + i, stringDeque[i][j], FOREGROUND_WHITE);
			if (stringDeque[i][j] > 0x7F) {
				offset += 1; //multibyte char use 2
			}
		}
	}
...
DisplayManager::DisplayManager(short _width, short _height) :width(_width), height(_height), currBufferIdx(1) {
	...
	borderline = height - textAreaHeight - 1;
...
void DisplayManager::WriteString(const std::wstring& s) {
	if (!stringSlowWrite.empty() && currTimeSlowWrite < totalTimeSlowWrite)
		return;
	...
	stringDeque.push_back(s);
	if (stringDeque.size() > textAreaHeight) {
